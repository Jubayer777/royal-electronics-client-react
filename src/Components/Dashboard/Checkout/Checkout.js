    ...
    const handlePaymentSuccess=(paymentResponse)=>{
        ...
        if(paymentResponse.startsWith('pm_')){
            ...
            if(cart.total !== 0){
                ...
                const newOrder= {user_id:userId, address:address, payment_id:paymentResponse,products:products,total:cart.total, status: 'Pending'}
                ...
                <form id="myForm">
                     <div className='name_email'>
                        <div className='book_half'>
                            ...
                            <br/>
                            <input  className='input_field double_field' onBlur={handleBlur} value={userName} name="name"  readOnly/>
                        </div>
                        <div className='book_half'>
                            <label className='input-label'>Email</label>
                            <br/>
                            <input  className='input_field double_field' onBlur={handleBlur} value={userEmail} name="email"  readOnly/>
                        </div>
                     </div>
                     <div className='address'>
                        <label className='input-label'>Address</label>
                        <br/>
                        <textarea className='input_field single_field' onBlur={handleBlur} placeholder="house#, road#, area#, district#" name="address" required/>
                     </div>
                 </form>
                   <div className='payment'>
                       <ProcessPayment  handlePayment={handlePaymentSuccess}></ProcessPayment>
                        <br />
                        <p className='pay_cost'>Total Cost:<span className="cost_amount">&#x9F3;</span>{cart.total}</p>
                        {
