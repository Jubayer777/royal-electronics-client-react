...
const Shop = () => {
    ...
    return (
        <div>
            ...
            <Navbar></Navbar>
            {
                    (products.length!==0 )&&<div className="product-container">
                
                        <div className="products">
                            {
                            
                                currentItems.map(pd => <Product key={pd.id} product={pd} brand={pd.brand} category={pd.category} handleAddProduct={handleAddProduct}></Product>)
                            }
                        </div>
                        <Pagination
             ...
             <CircleLoader color={color} loading={loading} css={override} size={60} />
                {
                    (products.length===0 && !loading)  &&  <h4 className="no-match"> No match found!</h4>
                }
             <Footer></Footer>
        </div>
    );
