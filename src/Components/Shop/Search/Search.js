...
const Search = () => {
    ...
    useEffect(()=>{
          fetch(`http://127.0.0.1:8000/api/p1/products/search/${searchData}`)
          .then(res=>res.json())
          .then(data=>{
    ...
    return (
        <div>
            ...
            <Navbar></Navbar>
            {
                (!error && products.length !==0)&& <div className="product-container">
                    <div className="products">
                        {
                        
                        !error && currentItems.map(pd => <Product key={pd.id} product={pd} brand={pd.brand} category={pd.category} handleAddProduct={handleAddProduct}></Product>)
                        }
                    </div>
                    <Pagination
